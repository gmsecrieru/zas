...
@test "http proxy: proxies a GET with an URL" {
  response="$(curl -s -H 'Host: simple.local' localhost:$ZAS_HTTP_PORT/path)"

  [ "$response" = "MOCK SIMPLE GET: Url: /path" ]
}

@test "http proxy: proxies a POST with an URL" {
  response="$(curl -s -H 'Host: simple.local' -X POST localhost:$ZAS_HTTP_PORT/path)"

  [ "$response" = "MOCK SIMPLE POST: Url: /path" ]
}

@test "http proxy: forwards POST data to the server" {
  response="$(curl -s -H 'Host: simple.local' -X POST localhost:$ZAS_HTTP_PORT/path -d some_data)"
